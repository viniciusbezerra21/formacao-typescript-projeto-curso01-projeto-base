function exibeHistorico(novaTransacao) {
    const registroDeTransacoes = document.querySelector("aside.extrato .registro-transacoes");
        ...
        <strong class="mes-group" style="color: var(--cor-secundaria); font-weight: 600; font-size: var(--fonte-t3);">${formatarData(novaTransacao.data, FormatoData.MES)}</strong>
                    <div class="transacao-item">
                        <div class="transacao-info">
                            ...
                            <strong class="valor">${formatarMoeda(novaTransacao.valor)}</strong>
                        </div>
                        <time class="data">${formatarData(novaTransacao.data, FormatoData.DIA_MES)}</time>
                    </div>
    `;
